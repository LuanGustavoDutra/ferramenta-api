      ...
      <div class="url-section">
        <div class="url-token-block">
          ...
          <div class="url-button-group">
            <button type="button" class="btn btn-danger" onclick="removerURL(this)"><i class="ri-close-line"></i></button>
          </div>
        </div>
  ...
  function removerURL(btn) {
    ...
    if (wrapper) {
      ...
      // Se não restar blocos, mostra o estado vazio e esconde o botão secundário
      if (urlCount === 0) {
        const emptyState = document.createElement("div");
        emptyState.className = "empty-state";
        emptyState.innerHTML = `
          <p>Nenhuma URL preenchida até o momento</p>
                <button class="btn-adicionar" id="toggle" onclick="adicionarURL()"><ix-icon name="add-circle-filled" class="btn-sm-icon"></ix-icon></i><div class="add-sm"> Adicionar</div></button>
        `;
        container.appendChild(emptyState);

        // Esconde o botão adicional
  ...
  async function enviarRequisicoes() {
    ...
    try {
      const response = await fetch("http://localhost:3000/executar-requisicoes", {
        method: "POST",
        headers: {
